...
## [1.9.3] - 2026-06-17
### Added
- Parent process verification `AAC_COMMIT_RUNNING` environment variable check in pre-commit hook and `helper.py commit` to block raw git commits.
- Base branch code modification validation check (Check 17) to prevent editing main/master directly.
...
- Comprehensive unit test `test_sync_remote_issue_rotation_on_failure` in `tests/test_issue_command.py` verifying Git config rotation and retry loop.
...
- Updated `.gitignore` to ignore the `sample/` directory and untracked existing review files.
...
[1.9.2]: https://github.com/rafaelghif/antigravity-agents/compare/v1.9.1...v1.9.2
