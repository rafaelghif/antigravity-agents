...
- Added `meeting_coordinator.py` for automated cron-based standups and blackboard synchronization.

### Changed
- Enhanced `manager_blindfold.py` with execution timeouts (>900s), strict reversion of stuck tasks, and unauthorized IO blocking.

## [4.34.0] - 2026-08-30
### Added
- Enforced RBAC Blindfold (`manager_blindfold.py`) via OS hooks to physically prevent the Scrum Master from bypassing delegation.
- Hardcoded `[OBLIGATORY_MEETING_PROTOCOL]` into `rules.md` (Procedural DNA) to ensure multi-agent meetings are never hallucinated across fresh installs.

### Fixed
- Corrected broken paths to `SKILL.md` triggers in `AGENTS.md` and injected `enable_write_tools` & `<EPISTEMIC_HUMILITY>` into all L9 personas.
- Expanded the Blackboard (`inbox_manager.py`) debate limit from 3 to 10 rounds to prevent premature blockages and forced hallucination.
