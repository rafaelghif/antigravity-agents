...
class TestInstallLifecycle(unittest.TestCase):
    def setUp(self):
        ...
        (self.src / ".agents" / "config.json").write_text('{"core_version": "4.47.2"}', encoding="utf-8")
    ...
    def test_install_generates_manifest(self):
        success = install_aac(self.target, "v4.47.2", source_override=self.src)
        self.assertTrue(success)

        manifest_file = self.target / ".agents" / "install_manifest.json"
        self.assertTrue(manifest_file.is_file())
        mdata = json.loads(manifest_file.read_text(encoding="utf-8"))
        self.assertEqual(mdata["installed_version"], "4.47.2")
        self.assertEqual(mdata["source_revision"], "v4.47.2")
    ...
    def test_audit_detects_tampered_files(self):
        install_aac(self.target, "v4.47.2", source_override=self.src)
