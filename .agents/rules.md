...
- **Git Auto-Closing & Remote-First MCP:** ALWAYS use MCP tools DIRECTLY to create issues, pull, merge, and manage remote PRs on GitHub/Gitea. DO NOT create offline `.agents/issues/` markdown files unless explicitly in offline mode. For PR creation, you MUST include `Fixes #<github_number>` in the PR body to enforce auto-closing via the Git platform conventional messages. If the MCP server crashes (e.g. `Bad Request`), fallback to GitHub CLI (`gh pr create`). Do NOT use local `helper.sh issue close` or edit `board.md` manually unless both MCP and CLI are unavailable. If offline, commit with `Closes #ID` to trigger local Git hooks.
...
- **Descriptive Execution:** The agent MUST explain its reasoning and actions explicitly to the user before execution. No silent background changes.
