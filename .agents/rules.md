...
- **Git Auto-Closing & Remote-First MCP:** ALWAYS use MCP tools DIRECTLY to create issues, pull, merge, and manage remote PRs. If the MCP server crashes (e.g. `Bad Request`), fallback to GitHub CLI (`gh pr create`). Do NOT use local `helper.sh issue close` or edit `board.md` manually unless both MCP and CLI are unavailable. If offline, commit with `Closes #ID` to trigger local Git hooks.
