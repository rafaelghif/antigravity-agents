...
- **Git Auto-Closing:** When finishing a task, do NOT manually edit `board.md` to move it to Done. Instead, simply commit with `Closes #ID` in the commit message. A local Git hook will automatically update the board and issue status offline. DO NOT use `helper.sh issue close` to bypass PRs unless explicitly instructed.
