...
## 1. Core Principles
* **Local Host Requirement**: Since Gitea is typically hosted locally (e.g., in a local network or containerized dev environment), you **MUST** configure `GITEA_HOST` to point to your specific local domain or IP address (e.g. `http://127.0.0.1:3000` or `https://git.local`) instead of the public `https://gitea.com` placeholder.
...
Run the Gitea MCP server container locally (make sure to set GITEA_HOST to your local Gitea URL):
```bash
docker run -i --rm -e GITEA_ACCESS_TOKEN=<YOUR_TOKEN> -e GITEA_HOST=http://localhost:3000 docker.gitea.com/gitea-mcp-server
```
