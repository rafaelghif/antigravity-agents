...
description: Writing documents for agents. Use when creating or editing Antigravity skills, rules, or AGENTS.md.
---

Reference for writing any document an agent consumes in Antigravity: a skill, `AGENTS.md`, a modular rule in `.agents/rules/`, or a doc reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable, since the agent takes the same _process_ every run rather than producing the same output. Note Antigravity's hard limit of 12,000 characters per rule file.
