---
name: git-workflow
description: Strictly handles the end-to-end Version Control Lifecycle (Issue -> Gitea Time Tracking -> Branch -> Atomic Commits -> PR -> Merge & Delete).
---
...
However:
- Still create a new branch, using the `quick-` prefix (e.g., `feature/quick-<slug>`).
...
- Determine if the origin is GitHub or Gitea.
- Create a new issue.
...
- Fallback: Use `tea` CLI if MCP is unavailable. If both are unavailable, PAUSE the workflow, use `ask_question` to notify the user, and do NOT proceed until tracking is resolved.
...
- Create a new branch.
- **Prefixes Allowed**: `feature/`, `bugfix/`, `hotfix/`, `chore/`, `refactor/`.
...
- **Message Format**: `<type>: <description> (Fixes #<issue_id>)`. Use `Fixes #` for GitHub and `Resolves #` for Gitea. If unsure, default to `Fixes #`.
...
- If `package-lock.json`, `pnpm-lock.yaml`, `yarn.lock`, `poetry.lock`, `Cargo.lock`, or `go.sum`: Accept the version from `main` and regenerate via the native package manager install command to deduplicate.
