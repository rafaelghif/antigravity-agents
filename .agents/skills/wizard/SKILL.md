...
The delightful UX is already solved by [template.sh](template.sh) (POSIX bash) and [template.ps1](template.ps1) (Windows PowerShell): stage-by-stage progress, confirmation gates, cross-platform URL opening, hidden secret entry, idempotent `.env` upserts, `gh secret`/`gh variable` writes, and a closing summary. **Your job is only to scope the procedure and author its stages.** The library above the `STAGES` marker is identical in every wizard; that consistency is the point: never hand-edit it.
