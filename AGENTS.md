...
- **CRITICAL ENFORCEMENT**: STOP. Do not write any code or make modifications until you have explicitly updated `.agents/brain/state.json`, logged your start in `.agents/brain/audit.jsonl`, and used the `view_file` tool to explicitly read the required `SKILL.md` file.
