...
- **UNIVERSAL AGENT REASONING PROTOCOL (UARP):** You **MUST ALWAYS** output a strict XML `<aac_preflight>` block in your visible response **BEFORE** executing any tools or modifying files. This guarantees cross-model reasoning consistency. The block MUST contain:
  `<aac_preflight>`
    `<audit>List target files to edit, active module locks, and matching schema boundaries.</audit>`
    `<compliance>Verify 10-year scale limits and confirm NO global/home-directory config leakage.</compliance>`
    `<action>State the exact CLI or code execution you are about to run.</action>`
  `</aac_preflight>`
  Never skip this XML block. It acts as the primary AI guardrail to eliminate context drift and hallucinations.
...
- **ALWAYS** perform a Pre-Implementation Impact Analysis comparing at least two options (following the `engineering-standards` playbook) to evaluate long-term maintenance, DRY principles, and simplicity *before* writing any code.
...
- **NEVER** write, expose, or leak project-specific configurations, specifications, plans, designs, database data, or artifacts to the global system level (such as the user's home directory, global agent appData/brain directories, or global databases). This strictly forbids caching skills playbooks, storing temporary files, exporting local credential keys, or saving execution reports outside the target repository boundaries (e.g. no writes to `~/.gemini/` or home folder directories except for static builtin read-only paths). The agent must operate strictly at the workspace level, keeping all inputs, outputs, and intermediate states completely isolated within the repository directory to ensure multi-developer alignment and prevent global environment leakage.
...
- **NEVER** loop or repeat tool calls, command executions, file checks, or code search patterns more than 3 times without making progress. If stuck, consult the `troubleshooting` skill; if still unresolved, halt and prompt the USER for manual intervention.
- **ALWAYS** check available skills in the prompt or `context_map.md` and load the required `SKILL.md` (via `view_file`) at the start of tasks matching that domain. **WARNING:** Do not rely solely on the brief skill descriptions injected in your prompt! Bypassing required playbook reads is strictly audited and blocks validation.
...
- **ALWAYS** enforce Zero-Trust Security: never commit secrets, never pull unverified remote scripts, strictly pin all package dependencies, and never execute obfuscated code (detailed in `security-compliance` playbook).
- **ALWAYS** implement robust error handling with tracebacks and never bypass static testing gates on base branches (detailed in `devops-release` playbook).
...
- **ALWAYS** act like an expert senior developer/software engineer: design with high foresight, keep cognitive complexity low, and prioritize standard, robust engineering patterns.
- **ALWAYS** verify standard library solutions or already installed dependencies first before writing new helpers or adding third-party packages, to prevent duplicate libraries and minimize codebase bloat.
- **ALWAYS** write enterprise-grade code that is clean, secure, performant, and scalable. Strict guidelines for Security, Performance (N+1 queries, memory leaks), Maintainability (SOLID principles), and 10-Year Architecture Foresight are offloaded to the `engineering-standards` and `security-compliance` playbooks.
...
## 5. Working protocol
1. **Session Resumption & Account Switching:** When starting a new conversation session (or switching accounts), ALWAYS check `.agents/tasks/board.md` for active tasks in the `Doing` column before assuming a new task. If an active task exists, use `./helper.sh issue checkout <task-id>` to load its branch, run `./helper.sh context optimize`, and read `.agents/state/active_context.md` to precisely resume where the previous session left off. Never hallucinate or create a new issue for a task that is already in progress.
...
3. **Before coding:** read `.agents/tasks/board.md`, claim the task, move it to `Doing`, create/checkout a new branch for the task (e.g., `./helper.sh issue checkout <task-id>`), run `./helper.sh context optimize`, and read `.agents/state/active_context.md` to align on active scope, checklist, and rules.
4. **Compliance Audit:** Perform a Rule & Schema Compliance Audit (listing target files, locks, rules, and schema matching using `.agents/active_context.md` as the primary source) before proposing or writing code.
5. **Task Splitting:** The agent MUST split any assigned task into small, atomic subtasks (no more than 3-5 lines of code edits per step where practical) and list them under `Implementation Subtasks` in the local issue specification file (e.g. `.agents/issues/issue_xxx.md`).
6. **Context Insulation (Single subtask focus):** The agent MUST focus on exactly ONE atomic subtask at a time. After completing a subtask, the agent MUST run validation and make a git commit with a clear Conventional Commit message referencing the task ID. This keeps the prompt cache warm and prevents context pollution.
7. **Active Context Pruning:** Between subtasks, if files are added or rules are changed, the agent MUST run `./helper.sh context optimize` to prune stale context, keeping the active context token-efficient and eliminating hallucinations.
8. **Pre-Implementation:** Perform a Pre-Implementation Impact Analysis comparing at least two options (following the `coding-standards` playbook) to evaluate long-term maintenance and UI/UX simplicity.
9. **Before any architecture-affecting change:** pull `@.agents/memory/architecture.md` and check `.agents/memory/decisions/` for a relevant ADR.
10. **While working:** prefer invoking an existing skill over re-deriving a workflow from scratch. When a task matches a skill's description, load that skill's `SKILL.md` using `view_file` to align on execution steps, ensuring the skill is used efficiently.
11. **Before marking a task `Completed`:** run all tests and `./helper.sh validate` to verify compliance. Once validation passes, run `./helper.sh changelog` to update release history, switch back to the base branch (`main` or `master`), merge the feature branch cleanly, and delete the feature branch local/remote if required.
12. **End of session:** run `/sync-memory` to fold session learnings into memory and prune anything stale (see `.agents/workflows/sync-memory.md`).
