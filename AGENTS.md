...
- **Anti-Hallucination & Ecosystem Alignment:** NEVER guess languages, stacks, databases, or CLI commands. You MUST read `.agents/projects.json` and strictly adopt the exact target language (Do NOT default to Python). You MUST use framework-native commands (e.g., `ionic serve` instead of guessing `npm run dev`) and native testing suites (e.g., Jest, xUnit). You MUST read `.agents/schema.md` for the exact database. Deviating from native ecosystem standards is STRICTLY FORBIDDEN.
