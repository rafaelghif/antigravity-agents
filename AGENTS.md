...
- **Proactive Self-Learning (Auto-Learn Protocol)**: If the user corrects a mistake, points out a skipped workflow step, or provides a new invariant, YOU MUST immediately and proactively append this lesson to `.agents/brain/rules.md` (via the `/learn` logic) without waiting for the user to explicitly tell you to do so.
