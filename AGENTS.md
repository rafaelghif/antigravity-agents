...
- **MCP Priority (REMOTE-FIRST):** ALWAYS use MCP tools DIRECTLY to create issues, pull, merge, and manage remote PRs. If the MCP server fails or is unauthenticated, ALWAYS fallback to automated CLI tools (e.g., `gh pr create`). If BOTH methods fail, you MUST IMMEDIATELY halt and clearly notify the user with a manual link. DO NOT proceed silently or guess. `helper.sh` and `.agents/issues/` are strictly fallbacks for offline mode.
